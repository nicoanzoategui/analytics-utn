...
export async function GET(request) {
    ...
    try {
        ...
        const data = rows.map((row) => ({
            ...
            screenPageViewsPerSession: parseFloat(row.metricValues[3].value)
        }));

        return NextResponse.json(data);
    } catch (error) {
        console.error("GA4 Error Sources:", error);
        return NextResponse.json({ error: "Error al obtener fuentes", details: error.message }, { status: 500 });
    }
}
